...
def json2lmdb(folderNm: str, fileNm: str):
  """
  convert data file from json format to lmdb format

  Args:
    folderNm (str): name of folder where data is stored
    fileNm (str): name of file of both json and lmdb format
  """
  import json
  with open('{}/{}.json'.format(folderNm,fileNm), 'r') as jsonFl:
    dtList = json.load(jsonFl)
  #print('type: {}'.format(type(dtList)))
  write_lmdb('{}/{}.lmdb'.format(folderNm,fileNm),dtList,map_size=(1024 * 50) * (2 ** 20)) # 50G
  print('Done')
...
if __name__ == '__main__':
  ...
  print(sys.argv[1])
  print(sys.argv[2])
  json2lmdb(sys.argv[1], sys.argv[2])
