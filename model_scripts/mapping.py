...
class TaskSpec:
    ...
    def register_model(self, model_name: str, model_cls: Optional[Type[BaseModel]] = None):
...
class Registry:
    ...
    def register_task(cls,
        ...
        Args:
            ...
            models (Optional[Dict[str, BaseModel]]): The set of models that can be used for
        ...
        """
        #print('registering task: {}'.format(task_name))
        if dataset is not None:
            if models is None:
                models = {}
            task_spec = TaskSpec(task_name, dataset, num_labels, models)
    ...
    def register_task_model(cls,
        ...
        Args:
            ...
            model_name (str): Name of model to use when registering task, this
                ...
                command line.
            model_cls (Type[BaseModel]): The model to register.
    ...
    def get_task_model(cls,
                       ...
                       load_extra_config: Optional[PathType] = None,
                       pretrained_epoch: int = None) -> BaseModel:
        ...
        Args:
            ...
            load_extra_config (str, optional): A json config file of extra
                hyperparams besides configs in pretrained folder
        Returns:
            model (BaseModel): A TAPE task model
        """
        ...
        # load extra configs
        extra_config_dict = {}
        if load_extra_config is not None:
            with open(load_extra_config, "r", encoding='utf-8') as reader:
                extra_config_dict.update(json.load(reader))

        if load_dir is not None:
            model = model_cls.from_pretrained(load_dir, pretrained_epoch=pretrained_epoch, num_labels=task_spec.num_labels, **extra_config_dict)
        else:
