...
*TO be modified from pfam_pretrain_entropy_mask.py*

input files: 
  family-wise protein MSA in stockholm/fasta format
mask policy:
  mask unaligned positions?
output files: 
...
flags.DEFINE_string('DATA_PATH', 'data_process/pfam', 
  "Path for data files")

flags.DEFINE_string('STAT_PATH', 'data_process/pfam/stat_rp15',
  "Path for statistic files")

flags.DEFINE_string('OUTPUT_PATH',
'data_process/pfam/seq_masked_tfrecords_sig_domain_1dProfile_entropy_rp15',
...
flags.DEFINE_string('FLNM_PFAM_FILE', 'data_process/pfam/pfam_rp15_seqs_files_famAcc', 
...
def raw_fasta2dict(data_path, input_format, fileNm, famAcc, clanAcc):
  '''
  input: 
    -data_path: data dir
...
def proc_aligned_seq(aligned_seq, raw_seq):
  ...
  Returns:
   ...
   - pos_type_vec: vector with size [max_seq_length]
          non-negative{0,1,2...} value as indices for aligned residues from left to right
...
def create_masked_predictions(
  ...
  if mode == 'single':
    ...
    # num of aligned posits should match
    assert len(cand_indexes) == len(idx_list_entropy)
    ...
    for idx in idx_list_entropy:
      glo_idx = cand_indexes(idx) # get the idx of this aligned AA in processed seq
...
def order_posit_entropy(aa_1d_prob, famAcc):
  '''
  return index list of aligned positions ordered
  from lowest entropy to highest entropy
  '''
  aligned_posit_n = aa_1d_prob.shape[0]
  entropy_arr = np.sum(-1.*aa_1d_prob*np.log(aa_1d_prob), axis=1)
  ordered_idx = np.argsore(entropy_arr)
  # save entropy_arr and ordered_idx
  np.savetxt('{}data_process/pfam/Pfam-A.hmm_1d_profile_entropy/{}'.format(FLAGS.PROJ_DIR,famAcc), np.stack((entropy_arr, ordered_idx), axis=-1))
  assert len(ordered_idx) == aligned_posit_n
  return ordered_idx
...
def main(argv):
  ...
  #for l in range(1):
    ...
    idx_list_entropy = order_posit_entropy(aa_1d_prob,famAcc)
    ...
    seq_dict_list = raw_fasta2dict(DATA_PATH, IN_FORMAT, fileNm, famAcc_no_version, clanAcc)
    ...
    instances = create_training_instances(seq_dict_list, MAX_SEQ_LENGTH, MASK_PROB, MAX_MASK_PER_SEQ, 
                                          aa_1d_prob, idx_list_entropy, rng, single_domain=SIG_DOM)     
